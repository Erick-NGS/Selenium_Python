...
# Func para criar tabela e relatório final para enviar via email
def criar_tabela(itens, precos, links, orcamento_total):
    ...
    #Adicionando valor 'Item' a coluna de 'Itens'
    index = 2
    for item in itens:
        ws.cell(column=1, row=index, value=item)
        index+=1

    #Adicionando valor 'Preço' a coluna de 'Preços'
    index = 2
    for preco in precos:
        if preco == msg_erro_pesquisa or preco == msg_erro:
            ws.cell(column=2, row=index, value=f"{preco}")
        else:
            preco = str(preco)
            preco = preco.replace(".", ",")
    ...
    #Adicionando valor 'Link' a coluna de 'Links'
    index = 2
    for link in links:
        ws.cell(column=3, row=index, value=link)
        index+=1

    #Adicionando valor 'Orçamento' a coluna de 'Orçamento'
    index = 2
